...
"""
Params that worked well: mode = 1, n_samples = 720, time_bound = 8000, modulu_param = 14, learning_rate = 0.0001,
...
dividing n_samples by 10 helps with the bias fixing algo (modulu_param and time_bound must be *10 in this case)
"""
...
"""
# Data loader (after preparing the data, this part is always the same)
    ...
    if how_to_test == "2":
        ...
        with torch.no_grad():
            ...
            for images, labels in test_loader:
                ...
                _, predicted = torch.max(outputs.data, 1)  # max returns (value ,index) but only indexes are necessary
...
train_csv = np.loadtxt("training_set.csv", delimiter=",", dtype=np.float32)
cell_num_to_delete_train = (train_csv.shape[0] % n_samples)
prices_train = train_csv[cell_num_to_delete_train:, 2:]
h1 = int(prices_train.shape[0] / n_samples)
x_arr_train = np.reshape(prices_train, (n_samples, h1))
y_arr_train = x_arr_train[future_n,1 :h1 - 1].T

test_csv = np.loadtxt("testing_set.csv", delimiter=",", dtype=np.float32)
cell_num_to_delete_test = (test_csv.shape[0] % n_samples)
prices_test = test_csv[cell_num_to_delete_test:, 2:]
h2 = int(prices_test.shape[0] / n_samples)
x_arr_test = np.reshape(prices_test, (n_samples, h2))
y_arr_test = x_arr_test[future_n,1 :h2 - 1].T



training_x_tensor = torch.from_numpy(x_arr_train)
training_y_tensor = torch.from_numpy(y_arr_train)
testing_x_tensor = torch.from_numpy(x_arr_test)
testing_y_tensor = torch.from_numpy(y_arr_test)
...
"""
csv = np.loadtxt("eu_price_in_usd.csv", delimiter=",", dtype=np.float32)
training_set = csv[:7994853, :]
testing_set = csv[7994853:, :]
